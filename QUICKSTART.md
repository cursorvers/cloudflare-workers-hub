...
## freee Company ID について（補足）

`FREEE_COMPANY_ID` は **通常不要** です。
Worker は `GET /companies` で company_id を自動解決し、D1（`external_oauth_tokens.company_id`）に保存します。

どうしても手動で上書きしたい場合のみ、シークレット `FREEE_COMPANY_ID` を設定してください。
...
# ログ確認
wrangler tail orchestrator-hub --format pretty
```
...
Optional: PDF text extraction (default OFF). See `docs/RECEIPT_PDF_TEXT_EXTRACTION.md`.
...
| `FREEE_COMPANY_ID` | ✅ 任意 | 自動解決（必要なら手動設定） |
...
2. Workers ログを確認
   ```bash
   wrangler tail orchestrator-hub --format pretty | grep Gmail
   ```
...
**確認事項**:
1. company_id が解決できているか
   - `FREEE_COMPANY_ID` を設定していればそれが優先されます
   - 未設定なら `external_oauth_tokens.company_id`（D1）に保存されているか確認
...
1. **Workers ログ確認**: `wrangler tail orchestrator-hub --format pretty`
