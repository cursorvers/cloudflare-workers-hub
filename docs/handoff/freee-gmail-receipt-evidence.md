...
- デフォルトは **件名(subject)のみ** でキーワード検索(誤検出を抑える)
- 本文(body)まで含めた **全文検索(full_text)** は取りこぼしを減らせる一方、誤検出でfreeeに不要なDeal/証憑が増えるリスクがある

切り替え(Worker env):

- PDF添付検索: `GMAIL_RECEIPT_KEYWORD_SCOPE=full_text`
- HTML本文レシート検索: `GMAIL_HTML_RECEIPT_KEYWORD_SCOPE=full_text`
