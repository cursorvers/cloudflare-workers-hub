...
- Worker runtime state is now the source of truth for finance mutations.
  - D1 token scope is `tenant_id + provider + company_id`
  - `freee-mcp` write calls must not bypass Worker audit / tenant controls
  - For multi-company auth, start Worker OAuth with `?company_id=<freee_company_id>`
