...
const stuckTasksRule: InsightRule = {
  ...
  generate: (context, config) => {
    ...
    return {
      ...
      rationale: '長期間更新がないタスクはブロッカーや分割不足の可能性が高く、進捗を止めるため。',
...
const tooManyActiveTasksRule: InsightRule = {
  ...
  generate: (context, config) => {
    ...
    return {
      ...
      rationale: 'WIPが多いほど切り替えコストが増え、完了速度と品質が低下しやすいため。',
...
const completionCelebrationRule: InsightRule = {
  ...
  generate: (context, config) => {
    ...
    return {
      ...
      rationale: '達成を可視化して、次の行動の継続モチベーションに繋げるため。',
      confidence: 1.0,
      priority: 'low',
      actionable: false,
      source: 'rule-engine',
      ruleId: 'completion-celebration',
      relatedGoals: recentlyCompleted.map(g => g.id),
      createdAt: now,
    };
  },
};
...
const noProgressRule: InsightRule = {
  ...
  generate: (context, config) => {
    ...
    return {
      ...
      rationale: '更新が止まる局面ではブロッカーの顕在化や優先度の再調整が効果的なため。',
...
const duplicatePatternRule: InsightRule = {
  ...
  generate: (context) => {
    ...
    return {
      ...
      rationale: '重複は認知負荷と作業コストを増やし、進捗の分散を招くため。',
      ...
      relatedGoals: [goal1.id, goal2.id],
      createdAt: Date.now(),
    };
  },
};
